...
function MissedLectures() {
  ...
  // const schools = stateContext.data.lectures.map((item) => item.school);
  ...
  const notYetStarted = stateContext.data.lectures.map(
    (item) => item.notYetStarted
  );
  console.log("Not yet lectures", notYetStarted);
  ...
  const tbs = notYetStarted.map((lecture, i) => {
    return (
      <>
        <Tab.Pane
          key={i}
        ...
        >
          ...
          >
            <div style={{}}>
              ...
              <h5>{`Lectures that have not yet started in ${schools[i]} TODAY`}</h5>
            </div>
          ...
          <Card.Body>
            ...
            <div className="d-grid d-sm-flex">
              <p className="mg-b-20 mg-b-20">
                {`Lectures that have not started on ${new Date().toLocaleDateString()} Per School`}
              </p>
            </div>
